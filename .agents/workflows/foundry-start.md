...
The Foundry skill must be installed in your workspace:
```
├── .foundry/
│   ├── SKILL.md              ← master orchestrator
│   ├── prompts/              ← all review & consultation prompts
│   ├── templates/            ← workflow template, interview guide
│   ├── scripts/              ← section extraction scripts
│   ├── ethskills/            ← (Ethereum projects) pulled by .foundry/scripts/pull_ethskills.sh
│   └── tob-skills/           ← (smart contracts) pulled by .foundry/scripts/pull_tob_skills.sh
├── .agents/workflows/        ← slash command workflows (foundry-start, curate, qa, etc.)
```
...
6. **Ingest MEDIUM-impact files (if user selects any):**
   - For each file the user wants ingested:
     ...
     - Run Prior Context Ingestion (PCI steps 1-3 from .foundry/SKILL.md Phase A) against it
...
7. **Resume from checkpoint:**
   ...
   - Proceed to the appropriate phase in .foundry/SKILL.md
...
1. Read the master orchestrator:
   ```
   Read .foundry/SKILL.md in full — this is the master orchestrator for the entire process.
   ```

2. Execute the bootstrap phases in order as defined in `.foundry/SKILL.md`:
   - **Phase 0: DISCOVER** — Run product discovery using `.foundry/prompts/office_hours.md`. Adapts to startup or builder context. Output `DESIGN_DOC.md`. Client must approve before proceeding.
   - **Phase A: CAPTURE** — If the user has existing PRDs/specs/context docs, run Prior Context Ingestion (PCI) first. Generate seed personas from design doc (for Ethereum projects, include an onchain seed persona and read ethskills context files before the interview), then run the deep interview using `.foundry/templates/interview_guide.md` with multi-perspective Advisory Mode. Run the CEO review gate using `.foundry/prompts/ceo_review.md`. If prior context docs were ingested, run the Reconciliation Gate to verify 100% content carry-over.
   - **Phase A½: SKILLS & WORKFLOWS** — Ask the user about existing skills/workflows to integrate. For Ethereum/onchain projects, read `.foundry/ETH-SKILL-GUIDE.md` and wire ethskills into the workflow and roadmap. For smart contract projects, also read `.foundry/TOB-SKILL-GUIDE.md` and wire Trail of Bits audit skills (dimensional analysis, spec-to-code compliance, entry point analysis, property-based testing, etc.) into the workflow and roadmap.
   - **Phase A¾: DESIGN** *(UI projects only)* — Run design consultation using `.foundry/prompts/design_consultation.md`. Output `DESIGN.md`.
   - **Phase B: SCAFFOLD** — Refine seed personas into full personas using `.foundry/prompts/crowe_persona_generator.md` with full interview context. For Ethereum projects, the onchain seed persona must be refined into a full Solidity/onchain specialist (mandatory for Step 3c sign-off).
   - **Phase C: STRUCTURE** — Extract sections using `.foundry/scripts/extract_sections.py`. Build index and implementation roadmap.
   - **Phase D: WORKFLOW** — Copy `.foundry/templates/workflow_template.md` to `PROJECT_WORKFLOW.md` and adapt for this project. Get user sign-off.
...
- All prompts in `.foundry/prompts/` are called at specific points — `.foundry/SKILL.md` tells you exactly when.
- **The Checkpoint Protocol in .foundry/SKILL.md governs checkpoint writes.** Every commit in the bootstrap sequence updates `.foundry/checkpoint.md`. This is what makes resume work.
