...
> Run this whenever a bug is encountered during implementation (Step 2) or when tests fail. This is a structured debugging methodology that prevents whack-a-mole fixing.
...
2. **Read the code:** Trace the code path from the symptom back to potential causes. Use grep to find all references, read files to understand the logic.
...
Also check:
- TODO/backlog files for related known issues
...
3. **3-strike rule:** If 3 hypotheses fail, **STOP.** Ask the client:
   ```
