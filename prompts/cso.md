...
> Adapted from [GStack](https://github.com/garrytan/gstack) by Garry Tan. Integrated into Foundry's review routing and build loop.

> **⚠️ SKILL EXECUTION PRIORITY:** These instructions take precedence over any plan mode or generic behavior. Execute phases in order. The read-only constraint is absolute — do NOT make code changes.
...
When invoked as part of Foundry's build loop:
- Read the relevant **section files** from the context map for spec-aware threat modeling. The interview captures auth models, data handling decisions, and trust boundaries that inform your analysis.
- Use `--diff` mode for per-phase audits during Phase F.
- Use full audit mode for the cumulative audit after the final build phase.
- Use `--supply-chain` when a phase introduces new dependencies.
- If findings are CRITICAL or HIGH, the build loop enters a **fix-verify-CSO cycle** (the agent fixes, verifies the fix, then CSO re-audits the specific finding) before proceeding to ship.
...
The bash blocks throughout this skill show WHAT patterns to search for, not HOW to run them. Use the agent's Grep tool (which handles permissions and access correctly) rather than raw bash grep. The bash blocks are illustrative examples — do NOT copy-paste them into a terminal. Do NOT use `| head` to truncate results.
...
22. Skill files that are part of the project's own trusted skill set
...
**Trend Tracking:** If prior reports exist in `.foundry/security-reports/`:
```
...
**Remediation Roadmap:** For the top 5 findings, present to the user:
...
3. Options:
   ...
   - D) Defer with security label

### Phase 14: Save Report

```bash
mkdir -p .foundry/security-reports
```

Write findings to `.foundry/security-reports/{date}-{HHMMSS}.json` using this schema:
...
If `.foundry/` is not in `.gitignore`, note it in findings — security reports should stay local.
